...
def run_real_data_backtest(data_file, model_file='xgb_model.json', conf_threshold=0.60):
    ...
    # 1. FEATURE ENGINEERING (Strictly from real data)
    ...
    for i in tqdm(range(len(df))):
        ...
        # LOGIC: Enter if AI is highly confident of an UP trend
        if position == 0 and conf > conf_threshold and sig == 1:
            position = balance / price
            balance = 0
                
        # EXIT: Real price drops below the short-term EMA trend line
        elif position > 0 and price < ema_21:
...
if __name__ == "__main__":
    # Test BTC
    run_real_data_backtest('BTCUSDT_2023_2024_June_15m.csv', conf_threshold=0.52)
    
    # Test ETH (Slightly lower threshold for higher volatility)
    run_real_data_backtest('ETHUSDT_2023_2024_June_15m.csv', conf_threshold=0.50)